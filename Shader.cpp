...
unsigned int Shader::compileShaders(std::string vertexSource, std::string geometrySource, std::string fragmentSource, std::vector<std::string> locations){
     char* vertexShaderSource = loadFile(vertexSource.c_str());
     char* geometryShaderSource = loadFile(geometrySource.c_str());
     char* fragmentShaderSource = loadFile(fragmentSource.c_str());
     unsigned int vertexShader;
    vertexShader = glCreateShader(GL_VERTEX_SHADER);
    glShaderSource(vertexShader, 1, &vertexShaderSource, NULL);
